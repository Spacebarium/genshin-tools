...
drop_to_asc_boss_name_map = {v["material"]["name"]: k for k, v in ASCENSION_BOSS_DATA.items()}
id_to_asc_boss_name_map = {str(v["material"]["id"]): k for k, v in ASCENSION_BOSS_DATA.items()}

# pprint(id_to_asc_boss_name_map)
...
id_to_weekly_boss_material_map = {}
for boss in WEEKLY_BOSS_DATA.values():
    for mat_name, mat_data in boss["materials"].items():
        item_id = str(mat_data["id"])
        id_to_weekly_boss_material_map[item_id] = mat_name
...
version = requests.get(VERSION_URL).json()["version"]
print(f"Grabbing data from v{version}...\n")

CHARLIST_URL = f"https://api.lunaris.moe/data/{version}/charlist.json"
charlist = requests.get(CHARLIST_URL).json()

MATERIAL_LIST_URL = f"https://api.lunaris.moe/data/{version}/materiallist.json"
material_list = requests.get(MATERIAL_LIST_URL).json()

for char_id, release_patch in char_ids:
    game_id = 10000000 + char_id

    url = f"https://api.lunaris.moe/data/{version}/en/char/{game_id}.json"
    ...
    weapon_type = WEAPON_TYPE_MAP[info["weapon"]]
    rarity = 5 if "ORANGE" in info["rarity"] else 4

    ascension_boss_mat_id = list(data["ascension"]["elitemonster"])[0]
    ascension_boss_name = id_to_asc_boss_name_map[ascension_boss_mat_id]

    talent_leveling_data = data["skills"]["leveling"]
    talent_book_id = list(talent_leveling_data["books"])[-1]
    talent_name = material_list[talent_book_id]["enName"].split(" ")[-1]

    weekly_boss_mat_id = list(talent_leveling_data["weekly"])[0]
    weekly_boss_mat = id_to_weekly_boss_material_map[weekly_boss_mat_id]
    ...
    char_data = {
        ...
        **({"shortName": short_name} if short_name else {}),
        ...
        "materials": {
            "ascensionBoss": ascension_boss_name,
            "weeklyBossMaterial": weekly_boss_mat,
            "talentBook": talent_name,
        },
        # "assets": {
        #     "cardImg": charlist[str(game_id)]["CardImg"],
        #     "gachaImg": charlist[str(game_id)]["GachaImg"],
        # },
    }

    result.update({id: char_data})

    icon_filename = charlist[str(game_id)]["CardImg"]
    url = f"https://static.nanoka.cc/assets/gi/{icon_filename}.webp"
    ...
    cropped.save(icons_dir / f"{id}.webp")

# pprint(result)

with open("output-lunaris.json", "w", encoding="utf-8") as f:
